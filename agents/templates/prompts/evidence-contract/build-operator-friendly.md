Before starting, resolve `{PRODUCT_ROOT}` per `agents/docs/AGENT-USE.md` → Session Setup and echo its absolute path on your first turn; every command below assumes that resolution. This prompt encodes the build action under the feature evidence contract from `feature-evidence-package-standardization-plan-v2.md` (effective `2026-05-19`).
...
Set up the build run folder under the base run evidence profile (§8 of the v2 plan). `BUILD_RUN_FOLDER` is `{PRODUCT_ROOT}/planning-mds/operations/evidence/{BUILD_RUN_ID}/`. Create it and initialize the base run files from templates: `README.md`, `action-context.md`, `artifact-trace.md`, `gate-decisions.md`, an empty `commands.log` (JSONL), and an empty `lifecycle-gates.log`. The build run folder is NOT a feature evidence package — every feature you close in this build keeps its own package at `{PRODUCT_ROOT}/planning-mds/operations/evidence/{FEATURE_ID}-{slug}/{RUN_ID}/`.
...
- build re-closing a feature that already has an approved package → produce a NEW `{RUN_ID}` for that feature, set `RERUN_OF` appropriately, and patch the prior manifest to `superseded` at B4
